    ...
    match inputtype {
        InputType::MouseLeftDown => {
            if gui.tileset_list.scrollbar.in_scrollbar(systems, screen_pos) {
                gui.tileset_list.scrollbar.hold_scrollbar(systems, screen_pos.y);
                ...
                if gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.in_scrollbar(systems, screen_pos) {
                    gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.hold_scrollbar(systems, screen_pos.y);
                }
            }
                ...
                match gui.current_tab {
                    ...
                    TAB_PROPERTIES => {
                        ...
                        if gui.selected_dropbox >= 0 {
                            let click_button = gui.editor_selectionbox[gui.selected_dropbox as usize].click_list(systems, screen_pos);
                            if let Some(selection_index) = click_button {
                                gui.editor_selectionbox[gui.selected_dropbox as usize].switch_list(systems, selection_index);
                                ...
                                gui.editor_selectionbox[gui.selected_dropbox as usize].hide_list(systems);
                            }
                        }
                    },
                    _ => {},
        ...
        InputType::MouseLeftDownMove => {
            if gui.scrollbar.in_hold {
                ...
                if gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.in_hold {
                    gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.move_scrollbar(systems, screen_pos.y, false);
                    let scrollbar_value = gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.cur_value;
                    gui.editor_selectionbox[gui.selected_dropbox as usize].update_list(systems, scrollbar_value);
                    gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.set_hover(systems, screen_pos);
                }
            }
        }
        InputType::MouseMove => {
            ...
            if gui.current_tab == TAB_PROPERTIES && gui.selected_dropbox >= 0 {
                gui.editor_selectionbox[gui.selected_dropbox as usize].hover_list(systems, screen_pos);
                gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.set_hover(systems, screen_pos);
            }
        }
        InputType::MouseRelease => {
            ...
            if gui.current_tab == TAB_PROPERTIES && gui.selected_dropbox >= 0 {
                gui.editor_selectionbox[gui.selected_dropbox as usize].scrollbar.release_scrollbar(systems);
            }
        },
    }
}
    ...
    match gui.current_tab {
        TAB_ATTRIBUTE => {
            ...
            match attribute {
                MapAttribute::Warp(_, _, _, _, _) => {
                    if gui.selected_textbox >= 0 {
                        if gui.selected_textbox < 2 {
                            gui.editor_textbox[gui.selected_textbox as usize].enter_numeric(systems, event, 5, true);
                        } else {
                            gui.editor_textbox[gui.selected_textbox as usize].enter_numeric(systems, event, 5, false);
                        }
                        result = true;
                    }
                },
                MapAttribute::Sign(_) => {
                    if gui.selected_textbox >= 0 {
                        gui.editor_textbox[gui.selected_textbox as usize].enter_text(systems, event, 100);
                        result = true;
                    }
                },
                _ => {},
...
pub fn set_tab(systems: &mut DrawSetting, gui: &mut Interface, tab_index: usize, mapview: &mut MapView, tileset: &mut Tileset, gameinput: &mut GameInput) {
    if gui.current_tab != tab_index {
        ...
        systems.gfx.set_visible(gui.labels[LABEL_OPT_HEADER_TEXT], false);
            ...
            TAB_LAYER => {
                for index in 0..MAX_TAB_LABEL {
                    if index < MapLayers::Count as usize {
                        gui.tab_labels[index].init(systems, MapLayers::as_str(index as u32), 194.0);
                    }
                }
            ...
            TAB_ATTRIBUTE => {
                ...
                for index in 0..MAX_TAB_LABEL {
                    ...
                    if sel_index < MAX_ATTRIBUTE as usize - 1 {
                        gui.tab_labels[index].init(systems, MapAttribute::as_str(sel_index as u32 + 1), 180.0);
                    }
                }
                gui.tab_labels[0].set_select(systems, true);
                ...
                systems.gfx.set_visible(gui.labels[LABEL_OPT_HEADER_TEXT], true);
                ...
                for i in 0..7 {
                    ...
                    if i != 1 {
                        ...
                        let text_box = Textbox::new(systems,
                            ...
                            Vec2::new(60.0, 22.0), false, [0, 1]);
                        gui.editor_textbox.push(text_box);
                    }
                }
                gui.editor_textbox[0].input_text(systems, mapview.map_zone_setting[0].max_npc.to_string()); // Max Npc
                for i in 0..5 {
                    if mapview.map_zone_setting[0].npc_id[i].is_some() {
                        gui.editor_textbox[i + 1].input_text(systems, mapview.map_zone_setting[0].npc_id[i].unwrap().to_string());
                    }
                }
            },
            TAB_PROPERTIES => {
                ...
                gui.editor_button = vec![
                    Button::new(systems, systems.resource.option_button.allocation, "Save All Map",
                            Vec2::new(pos.x + 14.0, pos.y + 372.0), Vec2::new(172.0, 36.0),
                            [ORDER_OPTION_BUTTON, ORDER_OPTION_BUTTON_TEXT], 8.0, [0, 1]),
                    Button::new(systems, systems.resource.option_button.allocation, "Reset All Map",
                            Vec2::new(pos.x + 14.0, pos.y + 332.0), Vec2::new(172.0, 36.0),
                            [ORDER_OPTION_BUTTON, ORDER_OPTION_BUTTON_TEXT], 8.0, [0, 1]),
                    Button::new(systems, systems.resource.option_button.allocation, "Preference",
                            Vec2::new(pos.x + 14.0, pos.y + 292.0), Vec2::new(172.0, 36.0),
                            [ORDER_OPTION_BUTTON, ORDER_OPTION_BUTTON_TEXT], 8.0, [0, 1]),
                ];
    ...
    match attr {
        MapAttribute::Warp(_, _, _, _, _) => {
            ...
            for i in 0..5 {
                ...
                let textbox_pos = match i {
                    1 => { Vec3::new(
                        pos.x + 65.0, 
                        pos.y + 316.0, ORDER_ATTRIBUTE_TEXTBOX) },
                    2 => { Vec3::new(
                        pos.x + 65.0, 
                        pos.y + 292.0, ORDER_ATTRIBUTE_TEXTBOX) },
                    3 => { Vec3::new(
                        pos.x + 65.0, 
                        pos.y + 232.0, ORDER_ATTRIBUTE_TEXTBOX) },
                    4 => { Vec3::new(
                        pos.x + 65.0, 
                        pos.y + 208.0, ORDER_ATTRIBUTE_TEXTBOX) },
                    _ => { Vec3::new(
                        pos.x + 65.0, 
                        pos.y + 340.0, ORDER_ATTRIBUTE_TEXTBOX) },
                };
                gui.editor_textbox.push(Textbox::new(systems,
                        textbox_pos, Vec2::new(60.0, 22.0), false, [0, 1]));
            }
        ...
        MapAttribute::Sign(_data_string) => {
            ...
            let text = create_basic_label(systems, 
                        Vec3::new(pos.x + 10.0,
                            pos.y + 368.0, ORDER_ATTRIBUTE_LABEL),
                        Vec2::new(100.0, 20.0),
                        Color::rgba(180, 180, 180, 255));
            gui.editor_label = vec![
                systems.gfx.add_text(text, 1),
            ];
            systems.gfx.set_text(&mut systems.renderer, gui.editor_label[0], "Sign Text");
            gui.editor_textbox = vec![
                Textbox::new(systems,
                    Vec3::new(pos.x + 10.0, 
                        pos.y + 115.0, ORDER_ATTRIBUTE_TEXTBOX),
                    Vec2::new(180.0, 250.0), true, [0, 1]),
            ];
            // If data exist, place the data on textbox
            if !data.is_empty() {
