    ...
    match inputtype {
        InputType::MouseLeftDown => {
            ...
            if gui.preference.scrollbar.in_scrollbar(systems, screen_pos) {
                gui.preference.scrollbar.hold_scrollbar(systems, screen_pos.y);
            }
                ...
                if let Some(index) = click_button {
                    match index {
                        ...
                        _ => {
                            ...
                            // Apply settings
                            systems.gfx.set_color(mapview.selection_preview,
                                Color::rgba(
                            ...
                            systems.gfx.set_color(tileset.selection,
                                Color::rgba(
                    ...
                    PREF_TAB_GENERAL => {
                        if let Some(index) = gui.preference.is_coloreditor_open {
                            if gui.preference.in_color_selection(systems, screen_pos) {
                                ...
                                if gui.preference.click_color_selection_button(systems, screen_pos) {
                                    if let SettingData::ColorSelection(colorselection) = &mut gui.preference.setting_data[index] {
                                        ...
                                        systems.gfx.set_color(colorselection.image, 
                        ...
                        if let Some(config_index) = gui.preference.select_config(systems, screen_pos) {
                                ...
                                SettingData::Checkbox(checkbox) => {
                                    ...
                                    match config_index {
                                        GENERAL_HIDEFPS => {
                                            ...
                                            systems.gfx.set_visible(gui.labels[LABEL_FPS], !checkbox.is_select);
                                        },
                                        GENERAL_HIDETILEBG => {
                                            config_data.hide_tileset_bg = checkbox.is_select;
                                            systems.gfx.set_visible(gui.bg_layout[2], !checkbox.is_select);
                                        },
                                        GENERAL_HIDEMAPBG => {
                                            config_data.hide_mapview_bg = checkbox.is_select;
                                            systems.gfx.set_visible(gui.bg_layout[1], !checkbox.is_select);
                                        },
                                        _ => {},
                                    }
                                    gui.preference.hide_color_selection(systems);
        ...
        InputType::MouseLeftDownMove => {
            gui.preference.scrollbar.move_scrollbar(systems, screen_pos.y, false);
