            ...
            match gui.current_tool {
                ...
                TOOL_EYEDROP => {
                    ...
                    if let Some((x, y, tile)) = systems.resource.tile_location.get(&id) {
                        (got_x = *x, got_y = *y, got_tile = *tile, got_data = true);
                    }
                    if got_data {
                        ...
                        let (posx, posy) = (got_x / TEXTURE_SIZE, (MAX_TILE_Y - (got_y / TEXTURE_SIZE) - 1));
            ...
            if !is_scrollbar_in_hold(gui) {
                ...
                if gameinput.selected_link_map.is_some() {
                    ...
                    match gui.current_tab {
                        ...
                        TAB_PROPERTIES => {
                            gui.editor_selectionbox[0].switch_list(systems, mapview.fixed_weather as usize);
                        },
                        _ => {},
                    }
                }
