...
impl Tileset {
    pub fn new(resource: &TextureAllocation, renderer: &mut GpuRenderer) -> Self {
        let mut tilesheet = Tileset {
            ...
            selected_tile: 0,
            selection: Image::new(Some(resource.white), renderer, 1),
        ...
        for tiledata in &resource.tilesheet[tilesheet.selected_tile].tiles {
        ...
        tilesheet.map.pos = Vec2::new(11.0, 391.0);
        tilesheet.map.can_render = true;

        // Setup tile selection image settings
        // We set the selected tile at the very first tile
        tilesheet.selection.pos = Vec3::new(tilesheet.map.pos.x, tilesheet.map.pos.y + ((MAX_TILE_Y - 1) * TEXTURE_SIZE) as f32, 4.0);
