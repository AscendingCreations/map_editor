    ...
    pub fn set_hover(&mut self, systems: &mut DrawSetting, in_hover: bool) {
        if self.in_hover != in_hover {
            ...
            if !self.is_selected {
                ...
                let (size, mut uv) = (systems.gfx.get_size(self.image),
...
impl TilesetList {
    pub fn new(systems: &mut DrawSetting) -> Self {
        ...
        let max_view = std::cmp::min(systems.resource.tilesheet.len() as u32, MAX_VISIBLE_LIST) as usize;
        ...
        for index in 0..max_view {
            ...
            let mut image = Image::new(Some(systems.resource.tileset_list_select.allocation), &mut systems.renderer, 0);
            image.pos = Vec3::new(bg1.position.x + 3.0, bg1.position.y + 369.0 - (21.0 * index as f32), ORDER_TILESETLIST_BUTTON);
        ...
        let scrollbar = Scrollbar::new(systems,
            Vec3::new(bg1.position.x + 188.0, bg1.position.y + 389.0, ORDER_TILESETLIST_SCROLLBAR), scrollbar_value as usize, 377, 20, 0);

        // We set the default selected tileset
        selection_buttons[0].set_select(systems, true);

        let bg = vec![
            systems.gfx.add_rect(bg1, 0),
            systems.gfx.add_rect(bg2, 0),
        ];
    ...
    pub fn update_list(&mut self, systems: &mut DrawSetting) {
        ...
        for index in 0..max_view {
            ...
            systems.gfx.set_text(&mut systems.renderer, self.texts[index], 
    ...
    pub fn hover_selection(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        ...
        for index in 0..self.selection_buttons.len() {
            let (pos, size) = (systems.gfx.get_pos(self.selection_buttons[index].image),
