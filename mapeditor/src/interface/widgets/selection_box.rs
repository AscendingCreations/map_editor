...
impl ListText {
    pub fn set_hover(&mut self, systems: &mut DrawSetting, is_hover: bool) {
        ...
        if self.is_hover {
            systems.gfx.set_color(self.rect, Color::rgba(55, 55, 55, 255));
        } else {
            systems.gfx.set_color(self.rect, Color::rgba(35, 35, 35, 255));
        }
    }
    ...
    pub fn set_select(&mut self, systems: &mut DrawSetting, is_select: bool) {
        ...
        if self.is_select {
            systems.gfx.set_color(self.rect, Color::rgba(20, 20, 20, 255));
        } else {
            if self.is_hover {
                systems.gfx.set_color(self.rect, Color::rgba(55, 55, 55, 255));
            } else {
...
impl SelectionBox {
    pub fn new(systems: &mut DrawSetting, pos: Vec2, z_order: [f32; 6], width: f32, list: Vec<String>, render_layer: usize) -> Self {
        let mut rect0 = Rect::new(&mut systems.renderer, 0);
        // Dropdown Box
        rect0.set_position(Vec3::new(pos.x, pos.y, z_order[0]))
        ...
        // Dropdown Box Image
        let mut button_image = Image::new(Some(systems.resource.selection_drop_button.allocation),
        ...
        let scrollbar = Scrollbar::new(systems,
            Vec3::new(pos.x + width - 13.0, pos.y - 6.0, z_order[5]), left_over, 90, 20, render_layer);

        let mut rect1 = Rect::new(&mut systems.renderer, 0);
        rect1.set_position(Vec3::new(pos.x, pos.y - (list_size - 1.0), z_order[2]))
    ...
    pub fn set_hover(&mut self, systems: &mut DrawSetting, is_hover: bool) {
        ...
        self.is_hover = is_hover;
        if self.is_hover {
            systems.gfx.set_color(self.rect[0], Color::rgba(55, 55, 55, 255));
            let mut uv = systems.gfx.get_uv(self.button);
            uv.y = 24.0;
            systems.gfx.set_uv(self.button, uv);
        } else {
            systems.gfx.set_color(self.rect[0], Color::rgba(35, 35, 35, 255));
            let mut uv = systems.gfx.get_uv(self.button);
            uv.y = 0.0;
            systems.gfx.set_uv(self.button, uv);
        }
    ...
    pub fn set_click(&mut self, systems: &mut DrawSetting, is_click: bool) {
        ...
        if self.is_click {
            systems.gfx.set_color(self.rect[0], Color::rgba(20, 20, 20, 255));
            let mut uv = systems.gfx.get_uv(self.button);
            uv.y = 48.0;
            systems.gfx.set_uv(self.button, uv);
        } else {
    ...
    pub fn click_list(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
        ...
        for (index, list_text) in self.list_text.iter().enumerate() {
            let (pos, size) = (systems.gfx.get_pos(list_text.rect),
    ...
    pub fn switch_list(&mut self, systems: &mut DrawSetting, index: usize) {
        ...
        systems.gfx.set_text(&mut systems.renderer, self.text, &self.list[self.selected_index]);
    }
}