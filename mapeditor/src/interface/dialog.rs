...
impl DialogButton {
    pub fn new(systems: &mut DrawSetting,
        ...
        let mut img = Image::new(Some(systems.resource.dialog_button.allocation), &mut systems.renderer, 1);
...
impl Dialog {
    pub fn new(systems: &mut DrawSetting,
        ...
        // Message
        let mut msg_text = create_label(systems,
        ...
        let mut scrollbar = Scrollbar::new(systems, 
                            ...
                            scrollbar_amount, 75, 5, 2);
        if dialog_type == DialogType::TypeMapSave {
    ...
    pub fn click_buttons(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> DialogButtonType {
    ...
    pub fn update_list(&mut self, systems: &mut DrawSetting) {
        for index in 0..4 {
            ...
            if text_index < self.editor_data.len() {
                systems.gfx.set_text(&mut systems.renderer, self.content_text[index], &self.editor_data[text_index]);
            }
        }
    }
