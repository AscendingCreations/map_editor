...
impl MenuButton {
    ...
    pub fn set_select(&mut self, systems: &mut DrawSetting, is_selected: bool) {
        ...
        if self.is_selected {
            systems.gfx.set_color(self.image, Color::rgba(50, 50, 50, 255));
            systems.gfx.set_color(self.text, Color::rgba(180, 180, 180, 255));
        } else {
            systems.gfx.set_color(self.image, Color::rgba(50, 50, 50, 0));
            systems.gfx.set_color(self.text, Color::rgba(50, 50, 50, 255));
        }
    }
}
...
impl KeyList {
    pub fn new(systems: &mut DrawSetting, pos: Vec2, msg: &str, keystr: &str) -> Self {
        ...
        keybutton.set_size(key_label_size)
    ...
    pub fn set_hover(&mut self, systems: &mut DrawSetting, is_hover: bool) {
        ...
        if self.is_hover {
            systems.gfx.set_color(self.key_button, Color::rgba(180, 180, 180, 255));
            systems.gfx.set_color(self.key_string, Color::rgba(40, 40, 40, 255));
        } else {
            systems.gfx.set_color(self.key_button, Color::rgba(50, 50, 50, 255));
            systems.gfx.set_color(self.key_string, Color::rgba(180, 180, 180, 255));
        }
    }
}
...
impl Preference {
    pub fn new(systems: &mut DrawSetting) -> Self {
        ...
        window1.set_size(Vec2::new(120.0, window_size.y - 65.0))
        ...
        window2.set_size(Vec2::new(window_size.x - 170.0, window_size.y - 65.0))
            .set_position(Vec3::new(window_pos.x + 150.0, window_pos.y + 45.0, ORDER_PREFERENCE_MENU))
            .set_color(Color::rgba(70,70,70,255))
            .set_use_camera(true);
        let pos = Vec2::new(window2.position.x + window2.size.x - 10.0, window2.position.y + 2.0);
        let mut window3 = Rect::new(&mut systems.renderer, 0);
        window3.set_size(Vec2::new(8.0, window_size.y - 69.0))
        ...
        let mut buttons = [
            Button::new(systems, systems.resource.preference_button.allocation, "Cancel",
                        ...
                        [ORDER_PREFERENCE_BUTTON, ORDER_PREFERENCE_BUTTON_TEXT], 2.0, [2, 3]),
        ...
        let scrollbar = Scrollbar::new(systems,
                            Vec3::new(window2.position.x + window2.size.x - 11.0, 
                                            window2.position.y + window2.size.y - 5.0, ORDER_PREFERENCE_SCROLLBAR),
                            0, window2.size.y as usize - 10, 20, 2);
        ...
        let mut window =
                ...
                systems.gfx.add_rect(window3, 2)]; // Scrollbar BG
    ...
    pub fn hover_buttons(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        ...
        match self.selected_menu {
            PREF_TAB_KEYBIND => {
                self.key_list.iter_mut().for_each(|keylist| {
                    let (pos, size) = (systems.gfx.get_pos(keylist.key_button),
            ...
            PREF_TAB_GENERAL => {
                self.setting_data.iter_mut().for_each(|setting| {
                    match setting {
                        SettingData::Checkbox(checkbox) => {
                            let (pos, size) = (systems.gfx.get_pos(checkbox.window[0]),
                        ...
                        SettingData::ColorSelection(colorselection) => {
                            let (pos, size) = (systems.gfx.get_pos(colorselection.image),
                ...
                if let Some(index) = self.is_coloreditor_open {
                    if let SettingData::ColorSelection(colorselection) = &mut self.setting_data[index] {
                        if colorselection.color_editor.is_open {
                            let (pos, size) = (systems.gfx.get_pos(colorselection.color_editor.button.image),
                                                systems.gfx.get_size(colorselection.color_editor.button.image));
                            if (mouse_pos.x) >= pos.x
                                && (mouse_pos.x) <= pos.x + size.x
                                && (mouse_pos.y) >= pos.y
                                && (mouse_pos.y) <= pos.y + size.y {
    ...
    pub fn click_color_selection_button(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> bool {
        if self.selected_menu != PREF_TAB_GENERAL {
            return false;
        }
        if let Some(index) = self.is_coloreditor_open {
    ...
    pub fn update_key_list(&mut self, 
        ...
        let (wpos, wsize) = (systems.gfx.get_pos(self.window[2]),
            systems.gfx.get_size(self.window[2]));
        let pos = Vec2::new(wpos.x + 10.0,
                                (wpos.y + wsize.y) - 30.0);
        let key_text = 
            ...
            match &mut self.setting_data[config_index] {
                SettingData::ColorSelection(colorselection) => {
                    ...
                    for (index, textbox) in colorselection.color_editor.textbox.iter_mut().enumerate() {
                        let (pos, size) = (systems.gfx.get_pos(textbox.image),
    ...
    pub fn in_color_selection(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> bool {
        ...
        if let Some(index) = self.is_coloreditor_open {
            if let SettingData::ColorSelection(colorselection) = &mut self.setting_data[index] {
                let (pos, size) = (systems.gfx.get_pos(colorselection.color_editor.window),
...
pub fn open_preference_tab(preference: &mut Preference,
                            systems: &mut DrawSetting,
                            config_data: &mut ConfigData) {
    let (wpos, wsize) = (systems.gfx.get_pos(preference.window[2]),
                                systems.gfx.get_size(preference.window[2]));
    // Clear Data
    preference.setting_data.iter_mut().for_each(|data| {
        match data {
    ...
    match preference.selected_menu {
        ...
        PREF_TAB_GENERAL => {
            ...
            let pos: Vec2 = Vec2::new(wpos.x + 10.0,
                (wpos.y + wsize.y) - 30.0);
            preference.setting_data = vec![
                SettingData::Checkbox(Checkbox::new(systems, Vec2::new(pos.x, pos.y), "Hide FPS?", Vec2::new(wsize.x - 30.0, 20.0),
                        [ORDER_PREFERENCE_SETTING_IMG1, ORDER_PREFERENCE_SETTING_IMG2, ORDER_PREFERENCE_SETTING_TEXT], config_data.hide_fps, [2, 3])),
                SettingData::Checkbox(Checkbox::new(systems, Vec2::new(pos.x, pos.y - 24.0), "Hide Tileset Background?", Vec2::new(wsize.x - 30.0, 20.0),
                        [ORDER_PREFERENCE_SETTING_IMG1, ORDER_PREFERENCE_SETTING_IMG2, ORDER_PREFERENCE_SETTING_TEXT], config_data.hide_tileset_bg, [2, 3])),
                SettingData::Checkbox(Checkbox::new(systems, Vec2::new(pos.x, pos.y - 48.0), "Hide Map View Background?", Vec2::new(wsize.x - 30.0, 20.0),
                        [ORDER_PREFERENCE_SETTING_IMG1, ORDER_PREFERENCE_SETTING_IMG2, ORDER_PREFERENCE_SETTING_TEXT], config_data.hide_mapview_bg, [2, 3])),
                SettingData::ColorSelection(ColorSelection::new(systems, Vec3::new(pos.x, pos.y - 72.0, ORDER_PREFERENCE_SETTING_IMG1), 
                        Vec2::new(70.0, 20.0), config_data.map_selection_color, Some("Map Selection Color"), false, [2, 3])),
                SettingData::ColorSelection(ColorSelection::new(systems, Vec3::new(pos.x, pos.y - 96.0, ORDER_PREFERENCE_SETTING_IMG1), 
                        Vec2::new(70.0, 20.0), config_data.tile_selection_color, Some("Tile Selection Color"), false, [2, 3])),
            ];
