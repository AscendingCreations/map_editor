...
impl Interface {
    pub fn new(systems: &mut DrawSetting, config_data: &mut ConfigData) -> Self {
        // Load the texture
        let mut bglayout = Image::new(Some(systems.resource.bg_layout.allocation), &mut systems.renderer, 0);
        bglayout.pos = Vec3::new(0.0, 0.0, ORDER_BG);
        bglayout.hw = Vec2::new(949.0, 802.0);
        bglayout.uv = Vec4::new(0.0, 0.0, 949.0, 802.0);
        let mut mapviewbg = Image::new(Some(systems.resource.mapview_bg.allocation), &mut systems.renderer, 0);
        ...
        let mut tilesetbg = Image::new(Some(systems.resource.tileset_bg.allocation), &mut systems.renderer, 0);
        ...
        systems.gfx.set_visible(bg_layout[1], !config_data.hide_mapview_bg);
        systems.gfx.set_visible(bg_layout[2], !config_data.hide_tileset_bg);
        ...
        scrollbarbg.set_size(Vec2::new(8.0, 313.0))
        ...
        tab_opt_bg0.set_size(Vec2::new(200.0, 422.0))
        ...
        tab_opt_bg1.set_size(Vec2::new(200.0, 24.0))
        ...
        systems.gfx.set_text(&mut systems.renderer, labels[LABEL_TILESET], &systems.resource.tilesheet[0].name);
        systems.gfx.set_text(&mut systems.renderer, labels[LABEL_MAPNAME], "Map [ X: 0 Y: 0 Group: 0 ]");
        systems.gfx.set_text(&mut systems.renderer, labels[LABEL_TILEPOS], "Tile [ X: 32 Y: 32 ]");
        for index in 0..MapLayers::Count as usize {
            tab_labels[index].init(systems, MapLayers::as_str(index as u32), 194.0);
        }
        tab_labels[0].set_select(systems, true); // Set Ground as selected
        systems.gfx.set_visible(labels[LABEL_FPS], !config_data.hide_fps);
    ...
    pub fn click_tool_button(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
    ...
    pub fn reset_tool_button_click(&mut self, systems: &mut DrawSetting) {
        ...
        self.buttons.iter_mut().for_each(|button| button.set_click(systems, false));
    }
    ...
    pub fn hover_selectionbox(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        ...
        self.editor_selectionbox.iter_mut().for_each(|selection_box| {
            let (pos, size) = (systems.gfx.get_pos(selection_box.rect[0]),
    ...
    // This function check which buttons are within the click position and return the button index
    pub fn click_selectionbox(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
        let mut found_button = None;
    ...
    pub fn hover_tab_option(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) {
        ...
        match self.current_tab {
            TAB_LAYER | TAB_ATTRIBUTE | TAB_ZONE => {
                for index in 0..MAX_TAB_LABEL {
                    let (pos, size) = (systems.gfx.get_pos(self.tab_labels[index].button),
    ...
    pub fn click_tab_option(&mut self, systems: &mut DrawSetting, mouse_pos: Vec2) -> Option<usize> {
        match self.current_tab {
            TAB_LAYER | TAB_ATTRIBUTE | TAB_ZONE => {
                self.tab_labels
                    .iter()
                    .enumerate()
                    .filter(|(index, label)| {
                        let (pos, size) = (systems.gfx.get_pos(label.button),
                                            systems.gfx.get_size(label.button));

                        *index < MAX_TAB_LABEL as usize && label.visible
                            && mouse_pos.x >= pos.x
                            && mouse_pos.x <= pos.x + size.x
                            && mouse_pos.y >= pos.y
                            && mouse_pos.y <= pos.y + size.y
            ...
            match self.current_tab {
                TAB_LAYER | TAB_ZONE => {
                    if self.tab_labels[tab_index].visible {
                        ...
                        self.tab_labels[self.current_selected_area as usize].set_select(systems, false);
        ...
        for index in 0..MAX_TAB_LABEL {
            ...
            if sel_index < MAX_ATTRIBUTE as usize - 1 {
                if self.current_tab_data == sel_index as u32 {
                    self.tab_labels[index].update(systems, MapAttribute::as_str(sel_index as u32 + 1), true);
                    self.current_selected_area = index as i32;
                } else {
                    self.tab_labels[index].update(systems, MapAttribute::as_str(sel_index as u32 + 1), false);
                }
            }
        }
    }
    ...
    pub fn open_zone_settings(&mut self, systems: &mut DrawSetting, mapview: &mut MapView) {
        ...
        // Max NPC
        self.editor_textbox[0].input_text(systems, mapview.map_zone_setting[zone_index as usize].max_npc.to_string()); // Max Npc
        // NPC
        for i in 0..5 {
            if mapview.map_zone_setting[zone_index as usize].npc_id[i].is_some() {
                self.editor_textbox[i + 1].input_text(systems, mapview.map_zone_setting[zone_index as usize].npc_id[i].unwrap().to_string());
            } else {
...
fn create_tool_button(resource: usize, systems: &mut DrawSetting, index: usize, pos: Vec3, hw: Vec2, uv: Vec4) -> ToolButton {
